...
Gradient decent helps to figure out what is the downhill direction. This direction is the negative of the gradient of the cost function, and its lenght is a indicator of the slope's steepest. So the training process consists of calculating gradient, taking a small step downhill (updating weights) and just repeating that over and over. It is a way to converge towards some local minimum of a cost function. Another important thing to know is that the magnitude of each component of calculated gradient vector tells how sensitive the cost function is to each weight and bias. 
